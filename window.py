...
class window :
    def __init__(self):
        ...
        #Sprites conf
        self.rootPath = "C:/Users/Alexandre/Desktop/pa/sprites/"
        self.sheetList = [["objects.png",800,1024,25,32],["terrain.png",1024,992,32,31],["caracteres.png",1200,1200,32,32],["black.png",1,1,1,1]]
        self.mapList = [["template11.png","mapTemplate1",100,100]]
        self.backList = [["mapTemplate1.jpeg",3200,3200,100,100,"mapTemplate1Collision.png"]]#[["map_test.png",960,960,30,30]]#
        self.menuList = [["menu.png",[0,0,655,55],[0,55,655,110],[0,110,65,175],[0,175,505,190],[0,190,505,200]]]
        self.resampleType = 1
        self.root,self.width,self.height = self.setRoot(width,height,offset,0,False,fullscreen,True,True)       
        ...
        waitBetweenFrames = 0.000
        fpsList = []
        self.zoomHandObj = 0.7
        self.displayName = True
        self.generate_new_map = False
        ...
        self.menuImageList = []
        self.maxInventory = 12
        self.inventoryList = []
        self.distancePickup = 30
        self.dropCooldown = 0
        self.openImageFiles()
        self.mouseVal = [-1,-1,0]
        self.menuBarSel = 0
        ...
        self.weponUsedCooldownReset = 10
        self.backgroundId = -1
        self.objIdList = []
        #Characters
        #self.chara_list = self.getRandChList(20,[[1500,1500],[1700,1700]],self.backList[0][5])#self.getRandChList(3,[[500,1000],[1000,1500]],self.backList[0][5])
        #self.chara_list = self.getRandChList(100,[[200,3000],[200,3000]],self.backList[0][5])#self.getRandChList(3,[[500,1000],[1000,1500]],self.backList[0][5])
        self.chara_list = self.getRandChList(1,[[1500,2000],[1500,2000]],self.backList[0][5])#self.getRandChList(3,[[500,1000],[1000,1500]],self.backList[0][5])
        #self.chara_list = self.getAllChList()
        self.createMe(34,[1500,1500])#)#[25*32,34*32]
        #Hand objects
        #self.handObjList = generateHandObjList()
        #self.handObjList.append([100,1400,1400,0])
        self.objInit(3)
        ...
        self.shootWeaponsObj = [8,40,41,54,69,70,79,51]
        ...
        while True :
            ...
            if displayFPS :
                ...
                #print("Generated draw in "+str(round(timedraw-start,6))+"s  ("+str(round(1/max((timedraw-start),0.0001),3))+" FPS)")
                #print("Generated update in "+str(round(timeupdate-start,6))+"s  ("+str(round(1/max((timeupdate-start),0.0001),3))+" FPS)")
    ...
    def drawBundle(self) :
        if self.backgroundId == -1 :
            self.backgroundId = self.drawBackgroundSheet(self.backList[0],-self.offset[0],-self.offset[1])
        else :
            self.canvas.move(self.backgroundId, 0, 0)
        #print(self.backgroundId)
    ...
    def keyMouseAction(self) :
        ...
        self.changeDirCharacterList(0.05,self.chara_list,keyDir)
        
        me = self.getMe()
        self.healthPct = me.healthPct
        
        self.weponUseAtempt(me)
        self.damageCalculation(me)
        
        if drop and self.dropCooldown==0:
            obj = self.getHandObjFromInventory(self.menuBarSel)
            if obj!=None :
                obj[3] = -1
                obj[1] = me.posMap[0]
                obj[2] = me.posMap[1]
                dropIndex = self.inventoryList.index(self.handObjList.index(obj))
    ...
    def damageCalculation(self,me):
        self.moveBullets(me)
        damage = 0.3
        if self.weponUsedCooldown == self.weponUsedCooldownReset :
            for cha in self.chara_list :
                dist = math.sqrt(math.pow(cha.posMap[0]-me.posMap[0],2)+math.pow(cha.posMap[1]-me.posMap[1],2))
                if not cha.playerControled and dist<40 :
                    cha.healthPct = cha.healthPct - damage
                    print(cha.healthPct)
                    if cha.healthPct < 0 :
                        cha.dead = True
        ...
        if self.bulletNum>0 :
            for i in range(self.bulletNum) :
                ...
                if self.handObjList[i][3] == -3 :
                    ...
                    if self.objOutOfScreen(self.handObjList[i]) :
                        ...
                        for cha in self.chara_list :
                            ...
                            if not cha.playerControled and dist<20 :
                                self.handObjList[i][3]=-2
                                cha.healthPct = cha.healthPct - damage
                                if cha.healthPct < 0 :
                                    cha.dead = True
    ...
    def weponUseAtempt(self,me):
        ...
        if self.listener.mouseVal[0]!=-1 :
            obj = self.getHandObjFromInventory(self.menuBarSel)
            if obj != None :
                if obj[0] in self.shootWeaponsObj :
                    speed = 20
                    bulletDir = [0,0]
                    ...
                    if len(self.handObjList[self.activeBullet])==7 :
                        self.handObjList[self.activeBullet][5] = dir2
                    else :
                        self.handObjList[self.activeBullet].append(dir2)
                    self.activeBullet = self.activeBullet + 1
                    if self.activeBullet==self.bulletNum :
                        self.activeBullet = 0
                    weponUsedFlag = True
                elif obj[0] in self.meleWeponsObj :
                    weponUsedFlag = True
    ...
    def drawCharacter(self,characterSheetSettings,img,dispX,dispY,name="",pc=False) :
        ...
        if name!="" and self.displayName :
            bold = ""
            fontSize = int(8)#int(4 + self.zoom*3)
            if pc :
                fontSize = int(10)
                bold = "bold"
    ...
    def openBackgroundList(self) :
        for backNum in range(len(self.backList)) :
            img = Image.open(self.rootPath+self.backList[backNum][0]).convert("RGB")
    ...
    def loadImageHandObjList(self,characterSheetSettings,mirror=False) :
        for j in range(len(self.handObjList)) :
            ...
            #img5 = ImageTk.PhotoImage(imageSmall.rotate(-90))#
            ...
            #self.handObjList[j][4].append(img5)
    ...
    #Create characters
    def getRandCh(self,spawnRange=[[0,500],[0,500]],collisionMat=None) :
        canMove = False
        while not canMove :#
            ...
            cha = ch.character(int((random.random()*101)),randPos)
    ...
    def getRandChList(self,n,spawnRange=[[0,500],[0,500]],collisionMat=None) :
        ch_list = []
        for i in range(n) :
            ch_list.append(self.getRandCh(spawnRange,collisionMat))
        return ch_list
    ...
    def characterCanMove(self,c,collisionMat=None,future=False) :
        if collisionMat==None :
            ...
            #print([min(max(newcoo[0],0),colSizeX),min(max(newcoo[1],0),colSizeY)])
            if not collisionMat[min(max(newcoo[1],0),colSizeY)][min(max(newcoo[0],0),colSizeX)] :# and not collisionMat[newcoo[1]+1][newcoo[0]+1] and not collisionMat[newcoo[1]+1][newcoo[0]] and not collisionMat[newcoo[1]][newcoo[0]+1] :
                ...
                #c.movingDir = [c.movingDir[0]/2,c.movingDir[1]/2]
                return False
            
    def changeDirCharacterList(self,p,chList,direction=[0,0]) :
        for c in chList :
